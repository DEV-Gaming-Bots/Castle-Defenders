...
class Voting : Panel
{
	...
	public List<VoteIcon> MapIcons = new();
	...
	private VoteIcon AddMap( string fullIdent )
	{
		...
		icon = new VoteIcon( fullIdent );
		icon.AddEventListener( "onmousedown", () => VoteEntity.SetVote( fullIdent ) );
		Body.AddChild( icon );
